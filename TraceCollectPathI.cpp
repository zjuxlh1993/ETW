	...
	for (USHORT k = 0; k < ArraySize; k++)
	{
		...
		else
		{
			...
			if (ERROR_SUCCESS == status)
			{
				...
				case 51:
				{
						   if (wcscmp(L"SysCallAddress", (PWCHAR)((PBYTE)(pInfo)+pInfo->EventPropertyInfoArray[i].NameOffset)) == 0
							   )
						   {
							   //WideCharToMultiByte
							   int iTextLen = WideCharToMultiByte(CP_ACP, 0, wstring(pFormattedData).c_str(), -1, NULL, 0, NULL, NULL);
							   char* pElementText = new char[iTextLen + 1];
							   memset((void*)pElementText, 0, sizeof(char)* (iTextLen + 1));
							   ::WideCharToMultiByte(CP_ACP, 0, wstring(pFormattedData).c_str(), -1, pElementText, iTextLen, NULL, NULL);
							   string strText;
							   strText = pElementText;
							   delete[] pElementText;

							   strText = strText.substr(11, 7);

							   DWORD dname = string16ToDword(strText);
							   LPWSTR name = addressTosyscall(dname);
							   if (name != NULL)
							   {
								   USES_CONVERSION;
								   SystemFlag = true;
								   nextsystemcallexit.processID = ThreadIDtoPID_map[curPID[pEvent->BufferContext.ProcessorNumber]];
								   parm = "\"SystemCall:" + string(W2A(name)) + '\"';
								   systemcallexit = parm;
								   nextsystemcallexit.threadID = threadID_list[pEvent->BufferContext.ProcessorNumber];
							   }
						   }
		...
		if (!pidInWhitelist(CPID) && CPID == ProcessIDOfRat&&OPcode != 36 && (!(OPcode == 51 && parm == "")) || (OPcode == 34 && EventType != ""&& pEvent->EventHeader.ProviderId.Data1 == 1171836109))
		{
			...
			if (OutFile_Map.find(threadID) != OutFile_Map.end()) {
				*OutFile_Map[threadID] << messageBody << endl;
				...
				string FileName = "C:\\Users\\zjulist\\Desktop\\Dotfuscator\\DIA_findSymbolByVA\\DIA_findSymbolByVA\\DATA\\TheadId" + to_string(threadID) + ".output";
